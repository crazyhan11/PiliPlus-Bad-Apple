...
for library in libmbedtls libmbedcrypto libmbedx509; do
    versioned_library=$(find dist/lib -maxdepth 1 -type f -name "${library}.*.dylib" | head -n 1)
    ln -sf "$(basename "${versioned_library}")" "dist/lib/${library}.dylib"
done
...
for library in libmbedtls libmbedcrypto libmbedx509; do
    versioned_library=$(find "${OUTPUT_DIR}/lib" -maxdepth 1 -type f -name "${library}.*.dylib" | head -n 1)
    ln -sf "$(basename "${versioned_library}")" "${OUTPUT_DIR}/lib/${library}.dylib"
done

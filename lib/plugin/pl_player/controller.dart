...
class PlPlayerController with BlockConfigMixin {
  ...
  String get decoderDiagnostics {
    final player = _videoPlayerController;
    if (player is! NativePlayer) return 'Native player is unavailable.';
    ...
    return [
      'requested-hwdec: ${hwdec ?? "no"}',
      'hwdec-current: ${property("hwdec-current")}',
      'video-codec: ${property("video-codec")}',
      'video-decoder: ${property("video-decoder")}',
      'video-params: ${property("video-params")}',
    ].join('\n');
  }
  ...
  // AV1 VideoToolbox is available on supported iOS devices, but mpv's generic
  // `auto` can select dav1d first. Prefer the system decoder and retain a
  // fallback for streams that VideoToolbox cannot accept.
  late final String? hwdec = Pref.enableHA
      ? (Platform.isIOS ? 'videotoolbox' : Pref.hardwareDecoding)
      : null;
  ...
  Future<Player> _initPlayer() async {
    ...
    final opt = {
      ...
      if (Platform.isIOS) 'hwdec-codecs': 'h264,hevc,vp9,av1',
    };

...
class _VideoDetailPageVState extends State<VideoDetailPageV>
  ...
  double get fullScreenVideoHeight {
    // On iPhone the safe-area edge can be reported as `padding.top` after a
    // landscape rotation. Subtracting it shrinks the player container and
    // leaves a black strip along the opposite edge. System controls already
    // use ViewSafeArea, so the video itself can occupy the full viewport.
    if (Platform.isIOS) return maxHeight;
  ...
  Widget _childWhenDisabledLandscapeInner(bool isFullScreen) {
    ...
    final videoHeight = isFullScreen ? fullScreenVideoHeight : height;
    if (height > maxHeight) {
